...
from rubin_scheduler.utils import DEFAULT_NSIDE, Site, _angular_separation, _hpid2_ra_dec, angular_separation, hpid2_ra_dec

from rubin_scheduler.scheduler.utils import IntRounded


def hp_in_polygon(nside, ra, dec):
...
class EuclidOverlapFootprint(SkyAreaGeneratorGalplane):
    ...
    def add_euclid_overlap(
        ...
        full_map = hp_in_polygon(self.nside, euclid_contours["RA"], euclid_contours["dec"])

        # find which map points are inside the contour
        indx = np.where((full_map == 1) & (self.pix_labels == ""))[0]
        self.pix_labels[indx] = label
        for filtername in filter_ratios:
            self.healmaps[filtername][indx] = filter_ratios[filtername]
